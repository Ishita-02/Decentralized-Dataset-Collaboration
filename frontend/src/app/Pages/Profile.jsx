...
import { User, Dataset, Contribution, Verification } from "@/entities/all";
  ...
  const loadProfileData = async () => {
    try {
      ...
      const [datasets, contributions, verifications] = await Promise.all([
        Dataset.filter({ owner: currentUser.email }),
        Contribution.filter({ contributor: currentUser.email }),
        Verification.filter({ verifier: currentUser.email })
      ]);
