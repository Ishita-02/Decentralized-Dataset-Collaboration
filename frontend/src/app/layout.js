...
import { usePathname } from "next/navigation"; // Correct hook from Next.js
...
export default function AppLayout({ children }) {
  // CORRECTED: Replaced useLocation with usePathname
  ...
  const NavContent = () => (
    <>
      ...
      <div className="flex-1 p-4">
        <nav className="space-y-2">
          {navigationItems.map((item) => (
            <Link
              key={item.title}
              href={item.url} // CORRECTED: Changed prop 'to' to 'href'
              onClick={() => setMobileMenuOpen(false)}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 ${
                pathname === item.url // CORRECTED: Used pathname
      ...
      <div className="p-6 border-t border-white/10">
        <Link 
          href={createPageUrl("Profile")} // CORRECTED: Changed prop 'to' to 'href'
          onClick={() => setMobileMenuOpen(false)}
